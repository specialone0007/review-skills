...
| Fourteen concerns apply. Three are moves (R15, P1): `docs/setup.md` covers `setup` by its headings and belongs at `docs/getting-started/SETUP.md`; `docs/h3-style.md` covers `operate` and belongs at `docs/guides/OPERATIONS.md`; the root `CHANGELOG.md` is the changelog and belongs at `docs/CHANGELOG.md` (the history bucket holds one doc here, so it stays flat). The rest are uncovered (R12, warnings): the agent file, onboarding, develop, deploy (`Dockerfile`), testing (`package.json` has a test script), contribute (`LICENSE.md`), architecture, http (four routes), security (`passport` in `package.json`), purpose and pipelines (`bullmq` and `src/jobs/export-worker.js`) | `mini-app/docs/`, `mini-app/package.json`, `mini-app/src/routes/health.js`, `mini-app/src/jobs/export-worker.js`, `mini-app/CHANGELOG.md` | `docs-structure` (R12 and R15) |
| `passport`, `stripe` and `bullmq` are declared dependencies, and `src/jobs/export-worker.js` reads `REDIS_HOST`, so the evidence inventory reports an auth surface, one integration and a jobs surface; `docs-sync-audit` reports `REDIS_HOST` as undocumented | `mini-app/package.json`, `mini-app/src/jobs/export-worker.js` | `docs-structure` (evidence), `docs-sync-audit` |
