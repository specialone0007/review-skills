...
1. **Init.** For every entry in the JSON `init.files`: copy the named template from this skill's `references/templates/` verbatim (the index template is built into the checker's output; the manifest content is in the block); add the `index_rows` to the central index with state `skeleton`; hold the `front_door` block for the organise step, so the states it names are the ones fill produced; print the `print_only` starter "Docs routing" section for the maintainer to paste into `CLAUDE.md` or `AGENTS.md`. Do not write that file; it is theirs. Init never fills a skeleton in.
