...
1. Run the checker. Paths are relative to this skill's directory, which varies by host. Use `python` if `python3` is not on PATH.
   ...
   python <skill-dir>/scripts/docs_fill_gate.py --repo <scratch> --all --no-git-root --git-repo . --strict --fail-on-findings   # after fill; never --repo .
   ```
