...
| agent | `AGENTS.md` at the root (+ `CLAUDE.md` = `@AGENTS.md` when `.claude/` exists); `<unit>/AGENTS.md` for a unit whose manifest has scripts | always | packages, tree, ci, tests - a skeleton: the commands with their source (the agent file owns them; the README links here), conventions and gotchas as open questions, a link to the index; forty lines at most |
...
| plan | plans · `TASKLIST.md` + `tasklist/` + `ROADMAP.md` | plan-like docs, or a `plans`, `roadmap`, `tasklist` or `tasks` folder | tree (plan-like docs only; never as checkboxes) |
