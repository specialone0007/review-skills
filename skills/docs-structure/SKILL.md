...
3. Read the generator line. If `mkdocs.yml`, `mkdocs.yaml`, `docusaurus.config.*`, `astro.config.*`, `SUMMARY.md`, `_sidebar.md`, `.vitepress/`, `sidebars.*`, `hugo.toml`, `book.toml` or a Sphinx `conf.py` exists at the repo root or in a docs root, that tool owns navigation and URLs: R1, R4, R11 and R12 are off, R5 ignores site routes, R3 is report-only, fill does nothing and says so. Three of those names are ordinary words, and finding one switches four rules off, so `conf.py`, `SUMMARY.md` and `_sidebar.md` have to corroborate themselves: the config must read like Sphinx (`extensions`, `master_doc`, `html_theme`), and the summary or sidebar must be nav-shaped, mostly links. A file that only shares the name is not a generator and the line reads `none`.
