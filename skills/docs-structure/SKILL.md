...
The inventory says what the repo **is** (kinds: application, library, cli, infrastructure, docs-only, monorepo; a repo can be several) and what it **contains**. A concern applies when the inventory finds the thing it describes. Six apply to every repo with code: the README, the agent file, setup, onboarding, develop and purpose. `references/shape.md` is the full spec - one row per doc with its reader, what it owns, its sections and the scenario table per kind; this table is the summary.
