...
| R14 | the central index says where each doc stands in words the checker can read: `skeleton`, `draft`, `reviewed YYYY-MM-DD`, `stale YYYY-MM-DD` (`current` is accepted as `reviewed`); any other word, or a reviewed row with no date, warns | warning only | P3 |
