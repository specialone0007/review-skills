...
- Which docs a repo needs comes from its evidence inventory (`scripts/docs_evidence.py`): a table of concerns, each one earned by something the repo contains or held by every repo. Where each doc lives is fixed by the shape in `references/shape.md`: six buckets by reader intent (getting-started, guides, reference, explanation, plans, history), every doc always in its bucket folder. Fourteen are unconditional - the README, the agent file, setup, onboarding, the daily loop, testing, deployment, operations, contributing, what it is, the architecture, the configuration, the integrations, security - and every other concern has to be earned; an `unknown` ecosystem means fill writes only open questions; every default is overridable in the manifest. There is no compatibility mode: a doc that covers a concern at another path is a move (R15), and apply does the move.
- Never delete or reword prose a human wrote. Apply may reorder it and re-home it: `docs_restructure.py` fits an existing doc into its template's section order, each existing section under the template section its heading matches, text verbatim and only the heading renamed, sections that match nothing kept after in their order, and a README section a doc owns pasted into that doc with one line and a link left behind; the proof is that every content line of every input appears exactly as often across the outputs and every link resolves, or nothing is written. Fill writes only into template skeletons, and everything it writes is a marked draft that the checker counts until a person reviews it.
...
| R11 | the front door answers what this is, how to run it and, when a LICENSE file exists, where it is (advice; no file means the checker cannot tell private from public and says nothing), keeps no second home for a concern a dedicated doc owns (a README H2 of more than six lines whose words match `deploy`, `data`, `http`... while the doc exists warns: keep a line and a link, move the rest), and hands off to the index: the root README (manifest `frontDoor`) links the central index from a `Start here` section (three files in order: the README, the index, `AGENTS.md`; then a hand-off to ONBOARDING, which owns the reading order); no such section is a warning, and a README that links eight or more docs directly is a second index and gets a warning. The agent file is R11's too: `AGENTS.md` over `agentFileMaxLines` (default 40) non-blank lines warns, and a `CLAUDE.md` with content of its own beside it warns - it is the one line `@AGENTS.md` | on when a central index exists and no site generator is detected | P1 |
...
The inventory says what the repo **is** (kinds: application, library, cli, infrastructure, docs-only, monorepo; a repo can be several) and what it **contains**. A concern applies when the inventory finds the thing it describes. Fourteen apply to every repo with code: the README, the agent file, setup, onboarding, develop, testing, deploy, operate, contribute, purpose, architecture, configuration, integrations and security. `references/shape.md` is the full spec - one row per doc with its reader, what it owns, its sections and the scenario table per kind; this table is the summary.

| concern | bucket · file | applies when | drafted from |
| --- | --- | --- | --- |
| readme | `README.md` at the root | always; a missing one is written from the template with the Start-here block in its slot, an existing one is never rewritten - the sections it lacks are R11 advice and `apply readme` appends them after the text | readme, packages, tree, services, env, ci, tests - What it is, Quickstart, Start here, Repository layout (table), Commands (a link to AGENTS.md), Configuration (a link), Status |
| agent | `AGENTS.md` at the root (+ `CLAUDE.md` = `@AGENTS.md` when `.claude/` exists); `<unit>/AGENTS.md` under every unit that deploys on its own | always | packages, tree, ci, tests - a skeleton: the commands with their source (the agent file owns them; the README links here), conventions and gotchas as open questions, a link to the index; forty lines at most |
...
| operate | guides · `OPERATIONS.md` | always; the health route or alert file found is the evidence, a library says it is not operated; per unit as for deploy | ops, jobs, services, env, decisions |
...
| contribute | root · `CONTRIBUTING.md` (listed under Guides; GitHub renders the root file in its Contributing tab) | always - how a change gets in exists in every team repo; with no LICENSE the doc says the repo is private and takes no outside contributions | tree, ci, tests, packages |
| release | guides · `RELEASING.md` | a publish or release script or workflow, or a release tool's config, any kind | release, packages, ci, decisions |
| architecture | reference · `ARCHITECTURE.md` (arc42-lite: context, containers, building blocks, runtime, deployment view, quality and risks) | always | packages, services, env, routes, schema, decisions |
| configuration | reference · `CONFIGURATION.md` | always; with nothing read, the doc says so | env, services, packages |
| data | reference · `DATA_MODEL.md` | schema or migrations exist, or a store SDK (Redis, S3, Mongo, Supabase, Firestore, DynamoDB, ...) | schema, decisions |
...
| plan | plans · `ROADMAP.md` | a doc with plan, roadmap or milestones in its name, or a `plans` or `roadmap` folder | tree (plan-like docs only; never as checkboxes) |
| tasks | plans · `TASKLIST.md` + `tasklist/` | a doc with todo, backlog, tasklist or tasks in its name, or a `tasklist` or `tasks` folder | never drafted; the phase table and one file per phase |

The "drafted from" column repeats each template's `<!-- concern: x; fill: ... -->` line, which is the authority. The path is `docs/<bucket>/<file>`, always (a repo whose docs live at the root keeps them flat there). `CONTRIBUTING.md` is the exception: it lives at the root, where GitHub renders it in the Contributing tab, and the index lists it under Guides. When the changelog lives under history the root also gets a one-line `CHANGELOG.md`, so GitHub surfaces it. The checker computes the path and reports it; nobody guesses.

**A monorepo keeps the map at the root and the steps in the unit.** The root DEPLOYMENT is a table of units and links; a unit with its own Dockerfile or platform config owns `<unit>/docs/DEPLOYMENT.md` (and `OPERATIONS.md` when it has a health route or cron); a unit never owns architecture, product, security or data. Package READMEs join the index under **Units**, one hop from it, and seed `setup` and `develop` the way the front door does - never own them.
...
1. Run the checker. Paths are relative to this skill's directory, which varies by host. Use `python` if `python3` is not on PATH.
   ...
   python <skill-dir>/scripts/docs_restructure.py --repo . --out <scratch>                 # every covered doc and the README into template order, with proof; proposes, never writes
   ```
...
3. **Organise.** Write the manifest the user accepted. **Restructure first:** `scripts/docs_restructure.py --repo . --format json` (add `--out <scratch>` for the gate) does the moves and the reshaping in one proposal. For every doc that covers a concern - at its path or about to move there - and for the README, it returns the doc in its template's order under `files` (a moved doc at its new path, the old one under `delete`): each existing section under the template section its heading matches, the text verbatim and only the heading renamed, template sections nothing matched with their guidance line, existing sections that matched nothing kept after in their order; a mapping marked `ambiguous` where two template sections matched equally, which a person confirms. The README's sections that a doc owns (an env table, deploy steps) are pasted into that doc's matching section as an H3 and replaced by one line and a link. Every Markdown file whose links pointed at a moved path is rewritten too. Write everything, and delete the moved-from paths, only when `proof.ok` is true: every content line of every input appears exactly as often across the outputs, every heading survives or is recorded as renamed, every link resolves. A split's parts folder moves with its doc through `scripts/docs_split.py --repo . --move <from> <to>`, which is also the tool for a single move without reshaping. Then insert the `front_door` block into the README when the init block carries one: after the intro paragraph under the H1, before the first H2, markers included, with each first stop's state read from the tree as it now stands (`fill` has run by this point, so a drafted doc says `draft`, not `skeleton`); on `refill` replace only what sits between the markers and leave the rest of the README alone. The central index is the list grammar of `references/shape.md`: an H1 `<project> docs`, one blockquote sentence, the intro, then one H2 per bucket that has docs - **Getting started**, **Guides**, **Reference**, **Explanation**, **Plans**, **History** - then any group a hand-made index already had, then **Units** (every unit README, unit agent file and unit doc) and **Root files**; under each, one line per doc: a dash, the doc as a link to its relative path, a colon, what it owns, a dash, its state. `init.files` carries the whole index whenever apply creates or rewrites it (a table-shaped index is an R15 finding and is rewritten, every row it had kept under its old heading in the new grammar), and `init.index_lines` carries the lines to add to one that already reads right - `skeleton` for a file init just created, `draft` for one fill wrote, `unreviewed` for a hand-written doc the index had not listed or a doc just moved; a commit is not a review, and `reviewed` is a person's act with a date (R14 reads these words; `none YYYY-MM-DD` is a person's confirmation that nothing applies). In a repo whose docs live at the root, the README is the index: append a `## Documents` list in the same line grammar when it has none, and add to that list otherwise. Add owner lines only to docs the user named: `> **This document owns:** <H1 text> *(auto, review me)*`. Split only the docs the user confirmed: run `scripts/docs_split.py --repo . --doc <path> --format json` for each. It follows `references/structure.md` exactly, returns the index and parts under `files`, the other Markdown files whose `X.md#anchor` links now point at a part under `inbound`, the non-Markdown files that name the old path under `mentions` (report them, never edit them), and runs the reversal proof itself under `proof`. Write the `files` and `inbound` contents only when `proof.ok` is true and `refuse` is null; `--out <scratch>` materialises the same tree outside the repository for the gate. A refusal is a finding ("needs a human restructure"), never a failure. The other direction is `docs_split.py --repo . --doc <index> --merge`: an older cut into confetti (R3 names the folder as a merge candidate) folds back into one doc, headings shifted back by the level the index records, links rebased, inbound links pointed at the whole, with the same kind of proof; write the doc and delete the parts only when `proof.ok` is true.
