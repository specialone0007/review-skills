...
| R11 | the front door answers what this is, how to run it and, when a LICENSE file exists, where it is (advice; no file means the checker cannot tell private from public and says nothing), keeps no second home for a concern a dedicated doc owns (a README H2 of more than six lines whose words match `deploy`, `data`, `http`... while DEPLOYMENT.md exists warns: keep a line and a link, move the rest), and hands off to the index: the root README (manifest `frontDoor`) links the central index from a `Start here` section (three files in order: the README, the index, the agent file if one is tracked; then the usual first stops); no such section is a warning, and a README that links eight or more docs directly is a second index and gets a warning | on when a central index exists and no site generator is detected | P1 |
...
**A monorepo keeps one doc per repo-wide concern.** When the inventory reports the `monorepo` kind, a package's own README (`server/README.md`, `agents/x/README.md`) seeds `develop`, `deploy`, `architecture` and `operate` the way the front door does, and never owns them: a three-service repository was told its Python service's README covered how the whole thing is run and deployed. The root doc keeps the map and one H3 per service; the package READMEs join the index under **Packages**, one hop from it. Ownership then runs one way: the root doc owns the repo-wide fact (every service's port, env names, deploy step), the package README keeps how to run that one package and links up to the root doc for the rest; a fact stated in both is the drift R8 exists to catch.
...
4. Draft-safety rules, gate-checked where mechanical:
   ...
   - DEPLOYMENT's Services section is the table the template asks for - one row per service: root, build, start, public or private, health - never a paragraph naming fifteen services; its per-service environment section is one H3 per service with every inventoried example file, one row per name, a flag column for a name the code reads that no example lists, and no name twice
