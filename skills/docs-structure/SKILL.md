...
| contribute | guides · `CONTRIBUTING.md` | always - how a change gets in exists in every team repo; with no LICENSE the doc says the repo is private and takes no outside contributions | tree, ci, tests, packages |
...
The "drafted from" column repeats each template's `<!-- concern: x; fill: ... -->` line, which is the authority. The path is `docs/<bucket>/<file>`, always (a repo whose docs live at the root keeps them flat there). The root also gets a one-line `CONTRIBUTING.md` and, when the changelog lives under history, a one-line `CHANGELOG.md`, so GitHub surfaces both. The checker computes the path and reports it; nobody guesses.
