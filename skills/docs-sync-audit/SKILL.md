...
- After the mandatory floor, one pass in the other direction, code to docs: list the subcommands and flags the argument parser defines (argparse, click, typer, cobra, clap, commander, yargs, System.CommandLine) and the public routes the router mounts, and check each against the CLI or API doc. One that no doc names is an `inferred` gap, filed as a finding with the parser or router line as its source.
- To deep-inspect a doc: for every command, path, variable name, version, route, port or default it states, and every statement of behaviour, permission or requirement it makes ("retries three times", "needs the admin role", "required in production"), and every function, class, method or option name in a code example, open the file that makes it true or false, and record the result under **Checks Run** as one line per doc: the doc, the claims you checked (a short phrase each, not a count, each followed by the file that settled it), and which of them mismatched. `mismatched: none` with no files named is a survey, not a deep inspection. For the names in an env sample the script's two-way comparison is the check, unless its no-reads warning fired; record it as `script pass` with the sample's path, and grep by hand only the names the script flagged. The sample's comments ("defaults to 30s", "required in production", "used by the worker only") are claims like any other and get the per-claim check. A doc with no such line was surveyed, not deep-inspected, and goes under **Surveyed But Not Deeply Inspected**. A script row you did not confirm by opening both sides goes there too, marked `script row, unconfirmed`, never into the numbered list.
...
- A `documented-unused-env` row: the script has already looked for the exact name in every non-doc file, so grep for the lower-case and camel-case forms (`database_url`, `databaseUrl`) in any settings, schema or config module, and for `${NAME}` in compose and deploy manifests. A hit means the script could not parse the read; drop the row. No hit and the doc says whose variable it is, or the name is a hosting platform's or a third-party tool's: P3, with that explanation. No hit and the name is the application's own: P2, the docs promise a knob that does not exist. A `documented-env-in-unreferenced-module` row is P2 once you have confirmed the module is unreachable, otherwise dropped.
- A `broken-link`, `missing-script`, `missing-make-target` or `missing-script-file` row is P1 when the reader must run that command or follow that link to install, run, test, upgrade or deploy; a command or link in the same doc that is not on that path (a code-of-conduct link, a storybook script under Extras) is P2. A confirmed `missing-path` lead is P2 when the reader must open the path to install, run, test, upgrade or deploy, otherwise P3. The test is per claim, not per doc; a README is not P1 all over because its install section is.
- P0 is only for a doc that, followed as written, damages a live system or exposes a secret: a deploy step that drops data, a runbook that prints a credential, a migration order that loses rows. A command that fails loudly, however early, is at most P1.
...
No code changed. I compared <source/code/change scope> against <docs checked>. <n> docs inventoried, <n> deep-inspected, <n> surveyed only. <verification summary>. No P0s found / P0s found: <count>.
...
**Checks Run**
- `<command>`: <result>
- `<doc>`: deep-inspected; claims checked: <phrase -> file that settled it, each>; mismatched: <which, or none>
