...
- What it does not check, so the report must not claim it did: `bundle exec`, `composer run`, and the `yarn dev` / `pnpm dev` shorthand without `run`; reStructuredText and MDX component links (only Markdown link syntax); a variable documented in a man-page source, a `--help` string or a generated site (it reads Markdown, MDX, reStructuredText and plain text only); a read whose name is on its platform and toolchain list (`NODE_ENV`, `PORT`, `GITHUB_TOKEN`, `AWS_SECRET_ACCESS_KEY`, `HF_TOKEN`, `GOOGLE_APPLICATION_CREDENTIALS` and the like; the count is `env_names_skipped_as_platform` in the totals); a read that happens only under an `examples/`, `fixtures/`, `samples/`, `demos/`, `benchmarks/`, `testdata/`, `tests/` or `specs/` folder; any shell-style read - `${NAME}`, `${NAME:?}`, `$NAME`, `$env:NAME` in a shell script, a Dockerfile, a compose file, a Makefile, a Helm values file or a Terraform variable; workflow YAML; a dot-folder other than `.github`. So an undocumented vendor credential and the no-default read that blocks a deploy are invisible to it. Grep the deploy files by hand (`grep -rn '\${[A-Z_]*' <deploy files>`) and compare the names against the deploy doc.
...
- Every row is a lead until you have opened both sides. Confirm the doc line and the source line yourself before it enters the report; the script's own footer says the same. Per kind:
  - `missing-script`: confirm whose project the command runs in. A library README telling its users to run `npm run build` in their own project is not a claim about this repository.
  ...
  - `documented-unused-env`: a lead with three endings, and the Severity Rubric says which one is which - the code reads the name through a mechanism the script cannot parse (a pydantic `Settings` field, a zod schema key, a Go struct tag, a compose `${NAME}`), the doc is describing someone else's variable, or the docs promise a knob the code does not have.
  - `documented-env-in-unreferenced-module`: config that reads as working in the docs but cannot take effect. Confirm the module really is unreachable before reporting it: the check uses name matching and cannot see dynamic imports.
...
- The script's `high` / `medium` / `low` are not P-levels. The mapping per kind:
  ...
  - `broken-link`, `missing-script`, `missing-make-target`, `missing-script-file`: P1 when the reader must run that command or follow that link to install, run, test, upgrade or deploy; a command or link in the same doc that is not on that path (a code-of-conduct link, a storybook script under Extras) is P2. The test is per claim, not per doc; a README is not P1 all over because its install section is.
  - `missing-path`, once confirmed: P2 when the reader must open the path to install, run, test, upgrade or deploy, otherwise P3.
...
- Any number you state -- matches, files, occurrences, endpoints -- must appear under **Checks Run** next to the command that produced it. Show the command and its result. If you are unwilling to show the command, do not state the number: describe the pattern instead. A count with no visible command behind it is the single easiest claim to get wrong, and forbidding it is not enough, so the rule is to evidence it or drop it. The header's three counts are the exception: they must equal the number of doc lines under **Checks Run** and the number of entries under **Surveyed But Not Deeply Inspected**, which is how a reader checks them.
...
- <Surfaces inventoried but not inspected deeply this pass; `partially inspected: <doc>` with its unchecked claims; `script row, unconfirmed: <kind> <doc>`; `floor not met: <doc>`; and which to run next. Keep the section in every mode; write `none` when nothing was left.>

**Checks Run**
- `<command>`: <result>
- `<doc>`: deep-inspected; claims checked: <phrase -> file that settled it, each>; mismatched: <which, or none>; unchecked: <phrases, or none>
