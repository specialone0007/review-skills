...
- What it does not check, so the report must not claim it did:
  ...
  - Every line of a fence after `cd /...`, `cd ~`, `ssh`, `docker exec` or `kubectl exec`: whatever runs there is not a path in this repository.
...
- `P1`: docs drift that blocks setup, release, API integration, migration, support, or a common user/admin workflow. "The setup path" below means the steps a reader follows to install, run, test, upgrade, deploy or migrate, and the API reference a client integrates against.
...
**Checks Run**
- `<command>`: <result>
- `<doc>`: deep-inspected; claims checked: <L<n> "phrase" -> file that settled it, each, in doc order>; mismatched: <which, or none>; unlocated: <phrases, or none>; unchecked: <phrases, or none>
