...
1. Establish the source of truth. Record `git status --short --ignored=matching` to a file outside the tree (`matching`, so a new file inside an already-ignored folder shows as its own line); rerun it at the end, diff the two, and report the diff under **Checks Run**: that diff, ignored files included, is the proof nothing was written. Run the script (`python <skill-dir>/scripts/docs_drift.py --repo <path> --format json`; the path is relative to this skill's own directory, which varies by host; use `python` if `python3` is not on PATH; a minute on a large repository is normal, do not kill it). Copy every string in its `warnings` array into **Not Tested**. Then the read-only checks under **Verify Safely**, output outside the tree. Locate manifests, scripts, routes, configs, schema files, migrations, API handlers, CLI entrypoints, env validation, generated-doc sources, and tests that reveal expected behavior.
...
3. The bounded script rows first, capped: forty `undocumented-env` rows, secret suffixes first, then twenty other rows; the rest wait for step 5, and the header says `<n> script rows deferred past step 3`. An `undocumented-env` row lists every place that reads the name under `readers`; open the window of each one and grade on the strictest, because the CLI's read may carry a default while the server's does not. The window is widened once when it shows neither a fallback nor a failure. Order them from what the JSON shows: `undocumented-env` rows whose name carries a secret suffix, then the other `undocumented-env` rows, then `missing-script`, `missing-make-target`, `missing-script-file` and `broken-link` rows in the README, env samples and the floor docs. Whether a read has a default is only visible in the source, and in Go, Node, Python and Rust the read line returns empty and the fatal check sits a few lines below, so open the readers in one batch as eight-line windows (`sed -n '<line>,<line+8>p' <path>` per reader, the command under **Checks Run**). No default means either the read form itself raises when the name is unset (`os.environ["X"]`, `ENV.fetch("X")`, `env!("X")`, `fetch_env!("X")`, `.expect` or `.unwrap` on the same line) or the read has no fallback argument and the lines that follow exit, throw, panic or return an error when it is empty; when the window shows neither a fallback nor a failure, widen it once to the enclosing function before deciding; still unclear, treat it as having a default, grade by name, and write `default: unclear` in the Evidence line. Promote a no-default read to P1 as you find it, under the rubric's path test. For an `undocumented-env` row the doc to open is the env sample or setup doc nearest the reading module; no env sample and no env table anywhere: the README is the expected area, and say so under Assumptions.
...
- What it does not check, so the report must not claim it did:
  ...
  - Links in reStructuredText and MDX component syntax (only Markdown link syntax is read), and among Markdown links any that starts with `../../`, a root-relative link with no file extension, a bare link with neither a slash nor a dot, a `::` rustdoc reference, a `/actions/workflows/` badge, a link to `docs/foo` that resolves only because `docs/foo.md`, `docs/foo/index.md` or `docs/foo/README.md` exists, and a link whose path contains a placeholder word (`example`, `foo`, `bar`, `path/to`, `your-`, `my-`), so a README link into `examples/` is opened by hand. The `#anchor` part of any link is stripped before the check, so a dead anchor after a heading rename never fires; `docs-structure` checks anchors. A package README below the repo root linking `../../CONTRIBUTING.md` is unchecked; open it by hand.
...
- Its doc inventory counts every `.txt` file, so `requirements.txt`, `LICENSE.txt` and `CMakeLists.txt` are in `docs_checked` and can carry a `stale-doc` row; list them under Assumptions as dropped, and keep `inventoried` equal to the `docs` array plus your additions so the sum closes.
...
No code changed. I compared <source/code/change scope> against <docs checked>. <n> docs inventoried, <n> deep-inspected, <n> partially inspected, <n> surveyed only. <the Verify Safely checks that ran, one phrase each, or `none run`>. No P0s found / P0s found: <count>; <n> P1, <n> P2, <n> P3; <n> script rows deferred past step 3.
