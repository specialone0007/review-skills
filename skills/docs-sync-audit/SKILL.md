...
- The script's `high` / `medium` / `low` are not P-levels. Map an env row by what the name is: an undocumented read whose name ends in `_KEY`, `_SECRET`, `_TOKEN`, `_PASSWORD` or `_DSN` is P1 unless the doc area already lists it under another name; a tuning knob (`_TIMEOUT`, `_LIMIT`, `_MS`, `_INTERVAL`) is P3; everything else starts at P2. A broken link or a missing script is P1 when it sits on the setup or deploy path, otherwise P2.
