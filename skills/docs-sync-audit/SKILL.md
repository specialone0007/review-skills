...
grep -ciE 'default|required|must|retries|role|only when' <doc>
```
...
- What it does not check, so the report must not claim it did:
  ...
  - Links in reStructuredText and MDX component syntax (only Markdown link syntax is read), and among Markdown links any that starts with `../../`, a root-relative link with no file extension, a bare link with neither a slash nor a dot, a `::` rustdoc reference, a `/actions/workflows/` badge, and a link whose path contains a placeholder word (`example`, `foo`, `bar`, `path/to`, `your-`, `my-`), so a README link into `examples/` is opened by hand. The `#anchor` part of any link is stripped before the check, so a dead anchor after a heading rename never fires; `docs-structure` checks anchors. A package README below the repo root linking `../../CONTRIBUTING.md` is unchecked; open it by hand.
...
Use this structure unless the user asks otherwise. Findings are numbered by P-level, P0 first, and by doc path within a level:
